...
import { CalendarIcon, X, Plus, Info, Trash2 } from "lucide-react";
...
export type CancelMode = "any" | "nonRefundable" | "freeBefore";

export interface FreeCancelTier {
  id: string;
  /** 提前多少小时之前免费取消 */
  beforeHours: number;
  /** 在该时间之后取消的服务费百分比 */
...
export interface CancelRuleConfig {
  id: string;
  channel: string;
  priceMin: number;
  priceMax: number;
  ...
  /** freeBefore 模式下的多档逻辑 */
  freeCancelTiers: FreeCancelTier[];
  priority: number;
}

export const DEFAULT_CANCEL_RULE: CancelRuleConfig = {
  id: "",
  channel: "",
  priceMin: 0,
  priceMax: 9999,
...
interface Props {
  ...
  initial?: CancelRuleConfig | null;
  onSave: (rule: CancelRuleConfig) => void;
}

export function CancelRuleDialog({ open, onOpenChange, initial, onSave }: Props) {
  const [rule, setRule] = useState<CancelRuleConfig>(DEFAULT_CANCEL_RULE);
  ...
  useEffect(() => {
    if (open) {
      setRule(
        initial
          ? { ...DEFAULT_CANCEL_RULE, ...initial }
          : { ...DEFAULT_CANCEL_RULE, id: `c${Date.now()}` }
      );
    }
  }, [open, initial]);

  const update = <K extends keyof CancelRuleConfig>(k: K, v: CancelRuleConfig[K]) =>
    setRule((r) => ({ ...r, [k]: v }));
  ...
  const addTier = () => {
    update("freeCancelTiers", [
      ...rule.freeCancelTiers,
      { id: `t${Date.now()}`, beforeHours: 24, feePercent: 0 },
    ]);
  };

  const updateTier = (id: string, patch: Partial<FreeCancelTier>) => {
    update(
      "freeCancelTiers",
      rule.freeCancelTiers.map((t) => (t.id === id ? { ...t, ...patch } : t))
    );
  };
  ...
  const removeTier = (id: string) => {
    update(
      "freeCancelTiers",
      rule.freeCancelTiers.filter((t) => t.id !== id)
    );
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-0 gap-0">
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <Section title="基础信息" description="选择规则适用的渠道和价格区间">
            <div className="space-y-4">
              <Field label="数据渠道" required>
                <Select value={rule.channel} onValueChange={(v) => update("channel", v)}>
          ...
          {/* 分区 2：适用条件 */}
          <Section title="适用条件" description="设置品牌与房型的匹配条件">
            <div className="space-y-4">
              <Field label="品牌关键词">
                <Input
                  className="h-10 text-sm"
          ...
          <Section title="日期设置" description="选择规则生效的日期范围与节假日策略">
            <Field label="日期模式">
              <RadioGroup
                value={rule.dateMode}
                onValueChange={(v) => update("dateMode", v as CancelRuleConfig["dateMode"])}
            ...
            <Field label="节假日">
              <RadioGroup
                ...
                onValueChange={(v) =>
                  update("holidayMode", v as CancelRuleConfig["holidayMode"])
                }
          ...
          {/* 分区 4：取消规则 */}
          <Section title="取消规则" description="设置取消政策与规则优先级">
            <div className="space-y-4">
              <Field label="取消政策" required>
                <RadioGroup
                  value={rule.cancelMode}
                  onValueChange={(v) => update("cancelMode", v as CancelMode)}
                  className="flex flex-col gap-2"
                >
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <RadioGroupItem value="any" /> 任意取消
                  </label>
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <RadioGroupItem value="nonRefundable" /> 不可取消
                  </label>
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <RadioGroupItem value="freeBefore" /> 限时免费取消
                  </label>
                </RadioGroup>

                {rule.cancelMode === "freeBefore" && (
                  <div className="mt-3 rounded-md border bg-muted/30 p-3 space-y-2">
                    <p className="text-xs text-muted-foreground">
                      可添加多档逻辑：在第一档时间之前免费取消；在该时间之后取消，按对应百分比收取服务费。
                    </p>
                    <div className="space-y-2">
                      {rule.freeCancelTiers.map((t, idx) => (
                        <div
                          key={t.id}
                          className="flex flex-wrap items-center gap-2 rounded border bg-background p-2.5 text-sm"
                        >
                          ...
                          <Input
                            ...
                            value={t.beforeHours}
                            onChange={(e) =>
                              updateTier(t.id, { beforeHours: Number(e.target.value) || 0 })
                            }
                          />
                          <span className="text-muted-foreground shrink-0">小时取消，收取</span>
                          <Input
                            type="number"
                            min={0}
                            max={100}
                            ...
                            value={t.feePercent}
                            onChange={(e) =>
                              updateTier(t.id, { feePercent: Number(e.target.value) || 0 })
                            }
                          />
                          <span className="text-muted-foreground shrink-0">% 服务费</span>
                          <Button
                            variant="ghost"
                            size="icon"
                      ...
                      {rule.freeCancelTiers.length === 0 && (
                        <p className="text-xs text-muted-foreground py-1">
                          暂无逻辑，点击下方添加
                        </p>
                      )}
                    </div>
                    <Button size="sm" variant="outline" className="h-8 text-xs" onClick={addTier}>
                      <Plus className="h-3.5 w-3.5 mr-1" /> 添加逻辑
                    </Button>
                  </div>
                )}
              </Field>
