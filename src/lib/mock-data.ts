import type { Hotel, Room, PriceRule, Order, Channel, HotelTag, Shop, ShopApiConfig, Operator, OtaAccount, DashboardStats } from './types';
...
export const mockHotels: Hotel[] = hotelNames.map((name, i) => {
  ...
  return {
    ...
    checkInTime: '14:00',
    checkOutTime: '12:00',
    hotelExternalId: String(14000000 + i * 1379),
    internalId: `${id}a7ea6c492f6f9a0e0237${i % 10}`,
    cityId: String(40 + (i % 50)),
    provinceId: '-',
    provinceName: ['上海', '北京', '浙江', '四川', '广东', '海南', '福建', '云南'][i % 8],
    countryId: '1',
    countryName: '中国',
    countryNameEn: 'China',
    countryType: '1',
    ...
    latitude: '-',
    starLevel: (i % 5) + 1,
    ratingDesc: ['超棒', '很好', '不错', '很棒'][i % 4],
    reviewCount: Math.floor(1000 + Math.random() * 90000),
    totalCount: Math.floor(500 + Math.random() * 3000),
    medalType: (i % 4) + 1,
    medalName: ['金特牌', '银特牌', '钻石牌', '铂金牌'][i % 4],
    email: '-',
    frontDeskHours: '前台营业时间：24小时营业',
    petPolicy: i % 2 === 0 ? '不可携带宠物' : '可携带小型宠物',
    destinationName: cities[i % cities.length],
    destinationNameEn: ['Shanghai', 'Beijing', 'Hangzhou', 'Chengdu', 'Shenzhen', 'Guangzhou', 'Sanya', 'Dali', 'Lijiang', 'Xiamen'][i % 10],
    timezoneOffset: '-',
  };
});
